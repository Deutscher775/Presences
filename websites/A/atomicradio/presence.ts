...
presence.on("UpdateData", () => {
	...
	if (player) {
		presenceData.largeImageKey = JSON.parse(
		...
		presenceData.smallImageText =
			player.querySelector("div.track-space").textContent; // Not in localStorage
		presenceData.buttons = [
			{
				label: "Listen",
				url: `https://atomic.radio/${
					player.querySelector("#spaceId").textContent // Not in localStorage
				}`,
			},
		];
